  ...
  <div class="w-full max-w-[450px] px-4 mx-auto py-10">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 md:p-8">
      <form @submit.prevent="registerUser" class="space-y-5">
        ...
        <div class="space-y-1">
          <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider ml-1">
            Full Name
          </label>
          <input
            v-model="name"
            ...
            required
          />
          <p v-if="fieldErrors.name" class="text-xs font-medium text-red-500 mt-1 ml-1">{{ fieldErrors.name[0] }}</p>
        </div>
        ...
        <div class="space-y-1">
          ...
          <input
            ...
            required
          />
          <p v-if="fieldErrors.email" class="text-xs font-medium text-red-500 mt-1 ml-1">{{ fieldErrors.email[0] }}</p>
        </div>
        ...
        <div class="space-y-1">
          ...
          <input
            ...
            :class="['form-input', { 'form-input-error': fieldErrors.password }]"
            required
          />
          <p v-if="fieldErrors.password" class="text-xs font-medium text-red-500 mt-1 ml-1">{{ fieldErrors.password[0] }}</p>
        </div>
        ...
        <div class="space-y-1">
          ...
          <input
            ...
            :class="['form-input', { 'form-input-error': fieldErrors.password_confirmation }]"
            required
          />
        </div>

        <div v-if="error" class="bg-red-50 dark:bg-red-900/30 border-l-4 border-red-500 p-3 rounded-r-lg animate-fade-in">
        ...
        <div class="flex items-start space-x-3 py-2">
          ...
          />
          <label for="terms" class="text-xs text-gray-500 dark:text-gray-400 leading-normal">
...
async function registerUser() {
  ...
  try {
    const res = await $fetch("/api/auth/register", {
      method: "POST",
      body: { name: name.value, email: email.value, password: password.value, password_confirmation: passwordConfirm.value },
    });
